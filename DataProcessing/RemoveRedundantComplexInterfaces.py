...
def is_equivalent(pdb1_filename, pdb2_filename, max_sequence_similarity):
    ...
    if percent_identity >= max_sequence_similarity:
        if percent_identity < 1:
            ...
            tm = ml.calculate_TMScore(pdb1_filename, pdb2_filename, os.path.join(temp_directory, "TM_Align.txt"), alignment=os.path.join(temp_directory, "Global_Alignment.fasta"))
...
def has_duplicate(pdb, pdb_list, max_sequence_similarity):
    global similar_chains_frame
    for other_pdb in pdb_list:
        eq, similar_chains_frame = is_equivalent(pdb, other_pdb, max_sequence_similarity)
        if eq:
            return True, similar_chains_frame
    return False, similar_chains_frame
...
if __name__ == "__main__":
    # Check arguments
    if len(sys.argv) != 3:
        print(f"Usage: $ python {os.path.basename(__file__)} <Species Root Directory> <Max Sequence Similarity>")
        sys.exit()
    ...
    sequence_cache = {}
    pdb_list = []
    nonredundant_chains_directory = os.path.join(root_directory, "..", "NonRedundant_Complex_Interfaces")
    similar_chains_path = os.path.join(root_directory, "..", "SimilarInterfaceComparisons.csv")
    ...
    for species_folder in os.listdir(root_directory):
        ...
        redundant_chains_directory = os.path.join(species_directory, "Separated_Complex_Interfaces")

        for cif_filename in os.listdir(redundant_chains_directory):
            print("     ", cif_filename)
            path = os.path.join(redundant_chains_directory, cif_filename)
            dup, similar_chains_frame = has_duplicate(path, pdb_list, max_sequence_similarity)
            if dup:
                pass
            else:
