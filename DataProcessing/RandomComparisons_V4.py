...
if __name__ == "__main__":
    ...
    output_frame = pd.DataFrame(columns=["PDB_ID_1","PDB_ID_2","Chain_1","Chain_2","Chain_1_Length","Chain_2_Length","Length_Difference","Alignment_Score","PID","TM"])
    ...
    for i in range(10000):
        ...
        # Save the data
        line = f"{pdb_id_a},{pdb_id_b},{chain_letter_a},{chain_letter_b},{chain_a_length},{chain_a_length},{nw_score},{percent_identity},{structure_similarity}"
        print("    "+line)
    ...
    for feature in ("Alignment_Score", "PID", "TM"):
        mean = np.mean(output_frame[feature])
        stdev = np.std(output_frame[feature])
        output_frame[f"{feature}_Normalized"] = (output_frame[feature] - mean)/stdev

    output_frame.to_csv(output_file_path)
