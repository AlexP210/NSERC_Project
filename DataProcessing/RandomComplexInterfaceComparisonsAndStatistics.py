...
if __name__ == "__main__":
    ...
    complex_interface_directory = os.path.join(root_directory, "..", "NonRedundant_Complex_Interfaces")
    output_file_path = os.path.join(root_directory, "..", "RandomComplexInterfaceComparisons.csv")
    ...
    complex_interface_files = os.listdir(complex_interface_directory)
    complex_interface_names = [cn.split(".")[0] for cn in complex_interface_files]
    ...
    for i in range(n_pairings):
        chain_idx_a = rand.randint(0, len(complex_interface_names)-1)
        chain_idx_b = rand.randint(0, len(complex_interface_names)-1)

        pdb_id_a = complex_interface_names[chain_idx_a][:4].lower()
        chain_letter_a = complex_interface_names[chain_idx_a][5].upper()
        chain_a_filename = f"{pdb_id_a}_{chain_letter_a}.ent"
        pdb_a_filename = f"{pdb_id_a}.cif"
        pdb_id_b = complex_interface_names[chain_idx_b][:4].lower()
        ...
        chain_a_path = os.path.join(complex_interface_directory, f"{pdb_id_a}_{chain_letter_a}.ent")
        chain_b_path = os.path.join(complex_interface_directory, f"{pdb_id_b}_{chain_letter_b}.ent")
    ...
    for species_folder in os.listdir(root_directory):
        ...
        heteromer_comparisons_path = os.path.join(species_directory, "Interfaces.csv")
        ...
        print("     Saving CSV")
        save_path = os.path.join(species_directory, "InterfacesWithSignificances.csv")
