...
if __name__ == "__main__":
    ...
    with open(chains_list_path, "r") as chains_list:
        with open(output_file_path, "w") as output_file:
            output_file.write("PDB_ID_1, PDB_ID_2, Chain_1, Chain_2, Percent_Identity_of_Global_Alignment, TM_Score, Identity_Of_Structural_Alignment, X_Rotation, Y_Rotation, Z_Rotation, Total_Rotation, X_Translation, Y_Translation, Z_Translation, Total_Translation\n")
            ...
            for chain_idx_a in range(len(chain_names)):
                for chain_idx_b in range(chain_idx_a):
                    ...
                    if not (os.path.isfile( os.path.join(pdb_directory, pdb_a_filename) ) and os.path.isfile( os.path.join(pdb_directory, pdb_b_filename) )):
                        ...
                        ananas_path = f"./Protein_Complexes"
                        ...
                        pairings_examined += 1
                        x_rot, y_rot, z_rot, rotation_angle, x_trans, y_trans, z_trans, trans_mag = calculate_rotation_angles("C:\\Users\\alexp\\Desktop\\School\\3rd_Year\\NSERC\\StructuralSequenceSimilarity\\Scripts\\Stuctural_Similarity\\matrix.txt")
                        # Write the output
                        print(f"Pairing #{pairings_examined}: {pdb_id_a}, {pdb_id_b}, {chain_letter_a}, {chain_letter_b}, {seq_score}, {TM_score}, {identity}, {x_rot}, {y_rot}, {z_rot}, {rotation_angle}, {x_trans}, {y_trans}, {z_trans}, {trans_mag}")
                        output_file.write(f"{pdb_id_a}, {pdb_id_b}, {chain_letter_a}, {chain_letter_b}, {seq_score}, {TM_score}, {identity}, {x_rot}, {y_rot}, {z_rot}, {rotation_angle}, {x_trans}, {y_trans}, {z_trans}, {trans_mag}\n")
