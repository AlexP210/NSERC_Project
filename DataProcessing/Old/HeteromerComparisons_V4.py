# Changes over V3:
# 1. Also calcualates the translation vector used to align the structures
...
if __name__ == "__main__":
    # Check arguments
    if len(sys.argv) != 5:
        print(f"\nUsage: $ python {__file__.split('/')[-1]} <PDB dir> <chains dir> <chains.txt> <output>\n")
        sys.exit()
            ...
            if not os.path.isfile(os.path.join(pdb_directory, f"pdb{pdb_id}.ent")):
                ...
                for sequence_a_idx in range(len(chain_sequences)-1):
                    for sequence_b_idx in range(sequence_a_idx+1, len(chain_sequences)):
                        ...
                        output_path = f"../../mnt/c/Users/alexp/Desktop/School/3rd_Year/NSERC/StructuralSequenceSimilarity/Scripts/DataProcessing/Temp/struc_alignment.txt"
                        matrix_path = f"../../mnt/c/Users/alexp/Desktop/School/3rd_Year/NSERC/StructuralSequenceSimilarity/Scripts/DataProcessing/Temp/matrix.txt"
                        ananas_output_path = f"../../mnt/c/Users/alexp/Desktop/School/3rd_Year/NSERC/StructuralSequenceSimilarity/Scripts/DataProcessing/Temp/ananas.txt"
                        tm_align_path = f"./Protein_Complexes/TMAlign/TMalign"
                        ananas_path = f"./Protein_Complexes/AnAnaS/ananas"
                        pdb_filename = f"../../mnt/e/NSERC_Data/PDB_Files/pdb{pdb_id}.ent"
                        call = f'ubuntu run "cd ~; {tm_align_path} {chain_a_path} {chain_b_path} -m {matrix_path} > {output_path}; {ananas_path} {pdb_filename} -C 100 c2 > {ananas_output_path}"'
                        os.system(call)
