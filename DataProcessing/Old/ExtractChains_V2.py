# Changes from V1:
#   1. Does not download PDBs AND split. Does only splitting, to allow cleaning up duplicates in between
...
if __name__ == "__main__":
    ...
    with open(chains_to_get_path) as pdb_textfile:
        for line in pdb_textfile:
            ...
            if os.path.isfile(os.path.join(pdb_directory, pdb_fn)) and not os.path.isfile( os.path.join(chains_save_path, f"pdb{pdb_id}_{chain}") ): 
                splitter.make_pdb(os.path.join(pdb_directory, pdb_fn), chain)