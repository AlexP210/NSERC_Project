        ...
        for pdb_id, chain_letters in id_to_chains.items():
            ...
            for sequence_a_idx in range(len(chain_letters)-1):
                for sequence_b_idx in range(sequence_a_idx+1, len(chain_letters)):
                    ...
                    #######################################
                    # CHANGE NEXT LINE BACK TO .CIF
                    ######################################
                    complex_path = os.path.join(assemblies_directory, f"{pdb_id}.ent")
                    # Percent identity
                    ...
                    # Symmetry RMSD
                    c2_rmsd, = calculate_symmetry_rmsd(complex_path, ["c2",], os.path.join(temp_dir, "AnAnaS_Output.txt"))
                    # Save the data
                    print(f"{pdb_id}, {chain_letters[sequence_a_idx]}, {chain_letters[sequence_b_idx]}, {len(load_sequence(chain_a_path))}, {len(load_sequence(chain_b_path))}, {percent_identity}, {structure_similarity}, {c2_rmsd}")
                    output_file.write(f"{pdb_id}, {chain_letters[sequence_a_idx]}, {chain_letters[sequence_b_idx]}, {len(load_sequence(chain_a_path))}, {len(load_sequence(chain_b_path))}, {percent_identity}, {structure_similarity}, {c2_rmsd}\n")
